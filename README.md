...
GitHub 웹 업로드 후에도 남을 수 있는 대시보드 파일 `http.py`, `scripts/sync_gamejob.py`, `tests/test_sync_gamejob.py`, `public/data/dashboard.json`은 저장소에서 삭제하는 것이 가장 안전합니다. 최신 Actions는 실행 전에 이 파일들을 임시 격리하여 표준 라이브러리 충돌과 지원자 데이터의 공개 배포를 방지합니다.
